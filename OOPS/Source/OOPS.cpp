...
float   OOPSClip(float min, float val, float max) {
...
int     OOPSIsPrime(uint64_t number )
{
...
//-----------------------------------------------------------------------------
double OOPSMtoF( double f )
{
    if( f <= -1500 ) return (0);
    else if( f > 1499 ) return (OOPSMtoF(1499));
...
{
    svf->k = 1.0f/OOPSClip(0.01f,Q,10.0f);
    svf->a1 = 1.0f/(1.0f + svf->g * (svf->g + svf->k));
    svf->a2 = svf->g * svf->a1;
    svf->a3 = svf->g * svf->a2;
...
{
    oops = (OOPS*) malloc(sizeof(OOPS));
    
    for (int i = 0; i < NI; i++)    oops->tPhasorRegistry[i]            = (tPhasor*) malloc(sizeof(tPhasor));
    for (int i = 0; i < NI; i++)    oops->tCycleRegistry[i]             = (tCycle*) malloc(sizeof(tCycle));
    for (int i = 0; i < NI; i++)    oops->tSawtoothRegistry[i]          = (tSawtooth*) malloc(sizeof(tSawtooth));
    for (int i = 0; i < NI; i++)    oops->tTriangleRegistry[i]          = (tTriangle*) malloc(sizeof(tTriangle));
    for (int i = 0; i < NI; i++)    oops->tSquareRegistry[i]            = (tSquare*) malloc(sizeof(tSquare));
    for (int i = 0; i < NI; i++)    oops->tNoiseRegistry[i]             = (tNoise*) malloc(sizeof(tNoise));
    for (int i = 0; i < NI; i++)    oops->tOnePoleRegistry[i]           = (tOnePole*) malloc(sizeof(tOnePole));
    for (int i = 0; i < NI; i++)    oops->tTwoPoleRegistry[i]           = (tTwoPole*) malloc(sizeof(tTwoPole));
    for (int i = 0; i < NI; i++)    oops->tOneZeroRegistry[i]           = (tOneZero*) malloc(sizeof(tOneZero));
    for (int i = 0; i < NI; i++)    oops->tTwoZeroRegistry[i]           = (tTwoZero*) malloc(sizeof(tTwoZero));
    for (int i = 0; i < NI; i++)    oops->tPoleZeroRegistry[i]          = (tPoleZero*) malloc(sizeof(tPoleZero));
    for (int i = 0; i < NI; i++)    oops->tBiQuadRegistry[i]            = (tBiQuad*) malloc(sizeof(tBiQuad));
    for (int i = 0; i < NI; i++)    oops->tSVFRegistry[i]               = (tSVF*) malloc(sizeof(tSVF));
    for (int i = 0; i < NI; i++)    oops->tSVFEfficientRegistry[i]      = (tSVFEfficient*) malloc(sizeof(tSVFEfficient));
    for (int i = 0; i < NI; i++)    oops->tHighpassRegistry[i]          = (tHighpass*) malloc(sizeof(tHighpass));
    for (int i = 0; i < NI; i++)    oops->tDelayRegistry[i]             = (tDelay*) malloc(sizeof(tDelay));
    for (int i = 0; i < NI; i++)    oops->tDelayLRegistry[i]            = (tDelayL*) malloc(sizeof(tDelayL));
    for (int i = 0; i < NI; i++)    oops->tDelayARegistry[i]            = (tDelayA*) malloc(sizeof(tDelayA));
    for (int i = 0; i < NI; i++)    oops->tEnvelopeRegistry[i]          = (tEnvelope*) malloc(sizeof(tEnvelope));
    for (int i = 0; i < NI; i++)    oops->tADSRRegistry[i]              = (tADSR*) malloc(sizeof(tADSR));
    for (int i = 0; i < NI; i++)    oops->tRampRegistry[i]              = (tRamp*) malloc(sizeof(tRamp));
    for (int i = 0; i < NI; i++)    oops->tADSRRegistry[i]              = (tADSR*) malloc(sizeof(tADSR));
    for (int i = 0; i < NI; i++)    oops->tEnvelopeFollowerRegistry[i]  = (tEnvelopeFollower*) malloc(sizeof(tEnvelopeFollower));
    for (int i = 0; i < NI; i++)    oops->tPRCRevRegistry[i]            = (tPRCRev*) malloc(sizeof(tPRCRev));
    for (int i = 0; i < NI; i++)    oops->tNRevRegistry[i]              = (tNRev*) malloc(sizeof(tNRev));
    for (int i = 0; i < NI; i++)    oops->tPluckRegistry[i]             = (tPluck*) malloc(sizeof(tPluck));
    for (int i = 0; i < NI; i++)    oops->tStifKarpRegistry[i]          = (tStifKarp*) malloc(sizeof(tStifKarp));
...
int OOPSSetSampleRate(float sampleRate)
{
    ...
    for (int i = 0; i < oops->registryIndex[T_PHASOR]; i++)         tPhasorSampleRateChanged(oops->tPhasorRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_CYCLE]; i++)          tCycleSampleRateChanged(oops->tCycleRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_SAWTOOTH]; i++)       tSawtoothSampleRateChanged(oops->tSawtoothRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_TRIANGLE]; i++)       tTriangleSampleRateChanged(oops->tTriangleRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_SQUARE]; i++)         tSquareSampleRateChanged(oops->tSquareRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_NOISE]; i++)          tNoiseSampleRateChanged(oops->tNoiseRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_ONEPOLE]; i++)        tOnePoleSampleRateChanged(oops->tOnePoleRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_TWOPOLE]; i++)        tTwoPoleSampleRateChanged(oops->tTwoPoleRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_ONEZERO]; i++)        tOneZeroSampleRateChanged(oops->tOneZeroRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_TWOZERO]; i++)        tTwoZeroSampleRateChanged(oops->tTwoZeroRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_POLEZERO]; i++)       tPoleZeroSampleRateChanged(oops->tPoleZeroRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_BIQUAD]; i++)         tBiQuadSampleRateChanged(oops->tBiQuadRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_SVF]; i++)            tSVFSampleRateChanged(oops->tSVFRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_SVFE]; i++)           tSVFEfficientSampleRateChanged(oops->tSVFEfficientRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_HIGHPASS]; i++)       tHighpassSampleRateChanged(oops->tHighpassRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_DELAY]; i++)          tDelaySampleRateChanged(oops->tDelayRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_DELAYL]; i++)         tDelayLSampleRateChanged(oops->tDelayLRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_DELAYA]; i++)         tDelayASampleRateChanged(oops->tDelayARegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_ENVELOPE]; i++)       tEnvelopeSampleRateChanged(oops->tEnvelopeRegistry[i]);
    //for (int i = 0; i < oops->registryIndex[T_ADSR]; i++)           tADSRSampleRateChanged(oops->tADSRRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_RAMP]; i++)           tRampSampleRateChanged(oops->tRampRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_ENVELOPEFOLLOW]; i++) tEnvelopeFollowerSampleRateChanged(oops->tEnvelopeFollowerRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_PRCREV]; i++)         tPRCRevSampleRateChanged(oops->tPRCRevRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_NREV]; i++)           tNRevSampleRateChanged(oops->tNRevRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_PLUCK]; i++)          tPluckSampleRateChanged(oops->tPluckRegistry[i]);
    for (int i = 0; i < oops->registryIndex[T_STIFKARP]; i++)       tStifKarpSampleRateChanged(oops->tStifKarpRegistry[i]);
...
int OOPS_tPhasorRegister(tTwoPole *o)
{
